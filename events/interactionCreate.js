...
return interaction?.reply({ content: "Tarif Limité.", ephemeral: true })
} else {
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            return interaction?.reply({ content: '🔴 Tarif Limité'.replace("{channel_filter}", channel_filter), ephemeral: true }).catch(e => { })
            }
        }
            }
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `<:PRO_PLUS:1222987767517024366> Rejoignez d'abord le canal vocal!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `<:PRO_PLUS:1222987767517024366> Doit être dans le même VC !!`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `<:PRO_RIGHT:1221477527449632779> Autorisations manquantes: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`, ephemeral: true });
            }
            } catch (e) {
            return interaction?.reply({ content: `<:PRO_OUT:1223462369154302043> Error...\n\n\`\`\`${e?.message}\`\`\``, ephemeral: true });
            }
            }
            });
            });
