...
module.exports = {
  name: "filter",
  description: "Ajoute un filtre audio à la musique en cours.",
  ...
  run: async (client, interaction) => {
    try {
      ...
      .setDescription('** Explorez le rythme, choisissez votre magie sonore ci-dessous!**')
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '<:PRO_OUT:1223462369154302043> Nom incorrect', ephemeral: true }).catch(e => { })
     filtre = filtre?.toLowerCase()
   
      if (filters?.includes(filtre?.toLowerCase())) {
        if (queue?.filters?.has(filtre)) {
          queue?.filters?.remove(filtre)
          embed?.setDescription(`Magie : **{filter}**, Statut appliqué: **{status}**`.replace("{filter}", filtre).replace("{status}", "<:PRO_OUT:1223462369154302043>"))
          return interaction?.editReply({ embeds: [embed] }).catch(e => { })
        } else {
          queue?.filters?.add(filtre)
          embed?.setDescription(`Magie : **{filter}**, Statut appliqué: **{status}**`.replace("{filter}", filtre).replace("{status}", "<:PRO_CHECKMARK:1222535886437224480>"))
          return interaction?.editReply({ embeds: [embed] }).catch(e => { })
        }
      } else {
        const filter = filters?.find((x) => x?.toLowerCase() === filtre?.toLowerCase())
        embed?.setDescription(`<:PRO_OUT:1223462369154302043> Impossible de trouver le filtre!!`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))
